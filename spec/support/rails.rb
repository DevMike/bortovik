# Custom matcher to ensure that the model have error on given field
RSpec::Matchers.define :have_errors_on do |attribute|
  ...
  match do |model|
    model.valid?
    @has_errors = !model.errors[attribute].empty?
  ...
  failure_message_for_should do |model|
    if @message
      "Validation errors #{model.errors[attribute].inspect} for '#{attribute}' should include #{@message.inspect}"
    else
